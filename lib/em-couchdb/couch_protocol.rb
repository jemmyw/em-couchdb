  ...
  module Protocols
    class CouchDB
      ...
      def get_all_dbs(&callback)
        http = EventMachine::HttpRequest.new("http://#{@host}:#{@port}/_all_dbs/").get :timeout => @timeout, :head => {"Content-Type" => "application/json"}
        http.callback {
      ...
      def create_db(db_name, &callback)
        http = EventMachine::HttpRequest.new("http://#{@host}:#{@port}/#{db_name}/").put :head => {"Content-Type" => "application/json"}
        if block_given?
          http.callback {
            callback.call()
          }
      ...
      def get_db(db_name, &callback)
        http = EventMachine::HttpRequest.new("http://#{@host}:#{@port}/#{db_name}/").get :timeout => @timeout, :head => {"Content-Type" => "application/json"}
        http.callback {
          callback.call(JSON.load(http.response))
        }
        http.errback {
      ...
      def delete_db(db_name, &callback)
        http = EventMachine::HttpRequest.new("http://#{@host}:#{@port}/#{db_name}/").delete :head => {"Content-Type" => "application/json"}
        if block_given?
          http.callback {
            callback.call()
          }
      ...
      def get(database, id, &callback)
        http = EventMachine::HttpRequest.new("http://#{@host}:#{@port}/#{database}/#{id}").get :timeout => @timeout, :head => {"Content-Type" => "application/json"} 
        http.callback {
          callback.call(JSON.load(http.response))
        }
        http.errback {
      ...
      def update(database, old_doc, new_doc, &callback)
        ...
        new_doc["_rev"] = rev
        new_doc["_id"] = id
        http = EventMachine::HttpRequest.new("http://#{@host}:#{@port}/#{database}/#{id}").put :body => JSON.dump(new_doc), :head => {"Content-Type" => "application/json"}
        http.callback {
          callback.call(JSON.load(http.response))
        }
        http.errback {
      ...
      def delete(database, doc, &callback)
        ...
        http = EventMachine::HttpRequest.new(URI.parse("http://#{@host}:#{@port}/#{database}/#{doc_id}?rev=#{doc_revision}")).delete :head => {"Content-Type" => "application/json"}
        http.callback{
          callback.call
        }
        http.errback {
